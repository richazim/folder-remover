# Change Log

All notable changes to the "folderremover" extension will be documented in this file.

Check [Keep a Changelog](http://keepachangelog.com/) for recommendations on how to structure this file.

## [Unreleased]

- Initial release
- 1.0.0
- 1.0.1
- 1.0.2
- 1.0.3
- 1.0.4:
  - Fix bug occuring when creating a folder in the vscode workspace project.
  - The extension start whenever VS Code starts up.
- 1.0.6:
  - Stable version.
- 1.0.7:
  - Fix: The extension can now force the deletion of a folder and retry 3 tree time if failed.
- 1.0.8
- 1.0.9:
  - Fixed readme video links
- 1.1.0:
  - Changelog: The extension can now conserve the unwanted uri if it is not deleted when vscode started.
- 1.2.0:
  The extension can now consider the opened instances of vscode. The activation of the extension will never try to remove the node_modules of the opened instances.
